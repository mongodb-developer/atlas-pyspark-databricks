...
add_books = my_spark.createDataFrame([("War and Peace", "Leo Tolstoy", 1867)], ["title", "author", "year"])
...
return_books = my_spark.read.format("com.mongodb.spark.sql.DefaultSource").option('uri', connection_string).option('database', 'bookshelf').option('collection', 'books').load()
